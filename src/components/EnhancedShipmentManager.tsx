...
type Recipient = ServiceCenter | Distributor
...
interface EnhancedShipmentManagerProps {
  brandId: string
  onShipmentCreated?: (result: any) => void
}

export default function EnhancedShipmentManager({ brandId, onShipmentCreated }: EnhancedShipmentManagerProps) {
  ...
  const [recipientType, setRecipientType] = useState<'SERVICE_CENTER' | 'DISTRIBUTOR'>('SERVICE_CENTER')
  ...
  const steps = [
    { number: 1, title: 'Select Recipient', icon: Users },
  ...
  const loadServiceCenters = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const formattedSCs = (data.serviceCenters || []).map((sc: any) => ({
          ...sc,
          type: 'SERVICE_CENTER' as const
        }))
        setServiceCenters(formattedSCs)
      }
    } catch (error) {
  ...
  const loadDistributors = async () => {
    try {
      const response = await fetch('/api/brand/authorized-distributors')
      if (response.ok) {
        const data = await response.json()
        const formattedDistributors = (data.distributors || []).map((dist: any) => ({
          ...dist,
  ...
  const handleRecipientSelect = (recipientId: string) => {
    const recipients = recipientType === 'SERVICE_CENTER' ? serviceCenters : distributors
  ...
  const calculateShippingCost = async () => {
    if (!selectedRecipient || boxAllocations.length === 0) return
    ...
    try {
      ...
      const response = await fetch('/api/shipments/cost-estimate', {
        ...
        body: JSON.stringify({
          ...
          shipments: [{
            ...
            recipientPincode: selectedRecipient.pincode,
      ...
      if (response.ok && data.estimates && data.estimates[0]) {
        ...
        const updatedBreakdown = {
          ...breakdown,
          insurance: insurance.type !== 'NONE' ? insurance : null,
          finalTotal: (breakdown.subtotal || 0) + (insurance.type !== 'NONE' ? insurance.total : 0)
        }
  ...
  const createShipment = async () => {
    if (!selectedRecipient || !costBreakdown) return
    ...
    try {
      ...
      const response = await fetch('/api/shipments/enhanced', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const canProceedToNextStep = () => {
    switch (currentStep) {
      case 1: return selectedRecipient !== null
  ...
  const currentRecipients = recipientType === 'SERVICE_CENTER' ? serviceCenters : distributors
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            Enhanced Shipment Manager
          </h2>
          <p className="text-muted-foreground">Send spares to authorized Service Centers & Distributors</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {currentStep === 1 && "Choose between Service Centers or Distributors, then select recipient"}
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <Label className="text-base font-medium">Select Recipient Type</Label>
                <Tabs value={recipientType} onValueChange={(value: 'SERVICE_CENTER' | 'DISTRIBUTOR') => {
                  setRecipientType(value)
                  setSelectedRecipient(null) // Reset selection when changing type
                }}>
                  <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="SERVICE_CENTER" className="flex items-center gap-2">
                      <Store className="h-4 w-4" />
                      Service Centers
                      <Badge variant="outline">{serviceCenters.length}</Badge>
                    </TabsTrigger>
                    <TabsTrigger value="DISTRIBUTOR" className="flex items-center gap-2">
                      <Building className="h-4 w-4" />
                      Distributors
                      <Badge variant="outline">{distributors.length}</Badge>
                    </TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
              ...
              {currentRecipients.length === 0 ? (
                <Alert>
                  ...
                  <AlertDescription>
                    You have no authorized {recipientType === 'SERVICE_CENTER' ? 'service centers' : 'distributors'}. 
                    Please add one in the Network tab first.
                  </AlertDescription>
                </Alert>
              ) : (
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <Label className="text-base font-medium">
                      Select {recipientType === 'SERVICE_CENTER' ? 'Service Center' : 'Distributor'}
                    </Label>
                    <Badge variant="secondary">
                      {currentRecipients.length} authorized
                    </Badge>
                  </div>
            ...
            <div className="space-y-4">
              ...
              {boxAllocations.length === 0 ? (
                <Alert>
                  ...
                  <AlertDescription>
                    Click "Next" to automatically allocate parts to boxes using AI optimization.
                  </AlertDescription>
                </Alert>
              ) : (
                <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <Label className="text-xs text-gray-500">
                            {selectedRecipient?.type === 'SERVICE_CENTER' ? 'Service Center' : 'Distributor'}
                          </Label>
                          <div className="flex items-center gap-2">
                            {selectedRecipient?.type === 'SERVICE_CENTER' ? (
                              <Store className="h-4 w-4 text-green-600" />
                            ) : (
                              <Building className="h-4 w-4 text-blue-600" />
                            )}
                            <div>
                              ...
                              <p className="text-gray-600">{selectedRecipient?.address}</p>
                            </div>
                          </div>
                        </div>
                        <div>
